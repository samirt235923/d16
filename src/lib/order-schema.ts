...
export type OrderStatus = 'new' | 'callPending' | 'callAgain' | 'confirmed' | 'processing' | 'deliveryPending' | 'outForDelivery' | 'done' | 'cancelled' | 'returned';
export type CallStatus = 'notCalled' | 'callPending' | 'callDone' | 'callAgain' | 'notInterested' | 'noAnswer';
...
export function calculateTotals(order: Partial<Order>) {
  ...
  const deliveryCharge = order.deliveryCharge || (order.area === 'inside' ? DELIVERY_INSIDE : DELIVERY_OUTSIDE);
